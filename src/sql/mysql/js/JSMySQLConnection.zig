...
_js_value: jsc.JSRef = jsc.JSRef.empty(),
_globalObject: *jsc.JSGlobalObject,
_vm: *jsc.VirtualMachine,
_poll_ref: bun.Async.KeepAlive = .{},
...
pub fn onAutoFlush(this: *@This()) bool {
    ...
    const keep_flusher_registered = this._connection.canFlush();
...
fn registerAutoFlusher(this: *@This()) void {
    if (!this.auto_flusher.registered and // should not be registered
        ...
        this._connection.canFlush())
    {
        AutoFlusher.registerDeferredMicrotaskWithTypeUnchecked(@This(), this, this._vm);
...
fn unregisterAutoFlusher(this: *@This()) void {
    if (this.auto_flusher.registered) {
        AutoFlusher.unregisterDeferredMicrotaskWithType(@This(), this, this._vm);
        this.auto_flusher.registered = false;
    }
}
...
fn stopTimers(this: *@This()) void {
    ...
    if (this.timer.state == .ACTIVE) {
        this._vm.timer.remove(&this.timer);
    }
    if (this.max_lifetime_timer.state == .ACTIVE) {
        this._vm.timer.remove(&this.max_lifetime_timer);
    }
}
fn getTimeoutInterval(this: *@This()) u32 {
    return switch (this._connection.status) {
        .connected => {
...
pub fn resetConnectionTimeout(this: *@This()) void {
    ...
    if (this._connection.status == .failed or
...
pub fn onConnectionTimeout(this: *@This()) void {
    ...
    if (this._connection.isProcessingData()) {
        return;
    }
...
pub fn enqueueRequest(this: *@This(), item: *JSMySQLQuery) void {
    ...
    this._connection.enqueueRequest(item);
    this.resetConnectionTimeout();
...
fn drainInternal(this: *@This()) void {
    debug("drainInternal", .{});
    if (this._vm.isShuttingDown()) return this.close();
    this.ref();
    defer this.deref();
    const event_loop = this._vm.eventLoop();
    event_loop.enter();
    defer event_loop.exit();
    this.ensureJSValueIsAlive();
    this._connection.flushQueue() catch |err| {
...
fn ensureJSValueIsAlive(this: *@This()) void {
    if (this._js_value.tryGet()) |value| {
        value.ensureStillAlive();
    }
}
pub fn finalize(this: *@This()) void {
    debug("finalize", .{});
    this._js_value.finalize();
    this.deref();
}
...
fn SocketHandler(comptime ssl: bool) type {
    return struct {
        ...
        pub fn onOpen(this: *JSMySQLConnection, s: SocketType) void {
            ...
            this._connection.setSocket(socket);
            ...
            if (socket == .SocketTCP) {
                ...
                this._connection.status = .handshaking;
        ...
        fn onHandshake_(
            ...
            const handshakeWasSuccessful = this._connection.doHandshake(success, ssl_error) catch |err| return this.failFmt(err, "Failed to send handshake response", .{});
            if (!handshakeWasSuccessful) {
                this.failWithJSValue(ssl_error.toJS(this._globalObject));
            }
        }
        ...
        pub fn onData(this: *JSMySQLConnection, _: SocketType, data: []const u8) void {
            ...
            this._connection.readAndProcessData(data) catch |err| {
                this.onError(null, err);
            };
        }

        pub fn onWritable(this: *JSMySQLConnection, _: SocketType) void {
            this._connection.resetBackpressure();
            this.drainInternal();
        }
    };
}
...
fn updateReferenceType(this: *@This()) void {
    if (this._connection.isActive()) {
        ...
        if (this._js_value.isNotEmpty() and this._js_value == .weak) {
            debug("strong ref until connection is closed", .{});
            this._js_value.upgrade(this._globalObject);
        }
        if (this._connection.status == .connected and this._connection.isIdle()) {
            this._poll_ref.unref(this._vm);
        } else {
            this._poll_ref.ref(this._vm);
        }
        return;
    }
    if (this._js_value.isNotEmpty() and this._js_value == .strong) {
        this._js_value.downgrade();
    }
    this._poll_ref.unref(this._vm);
}
...
pub fn createInstance(globalObject: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) bun.JSError!jsc.JSValue {
    ...
    ptr._connection.status = .connecting;
    ptr.resetConnectionTimeout();
    ptr._poll_ref.ref(vm);
    const js_value = ptr.toJS(globalObject);
    js_value.ensureStillAlive();
    ptr._js_value.setStrong(js_value, globalObject);
...
pub fn getConnected(this: *@This(), _: *jsc.JSGlobalObject) JSValue {
    return JSValue.jsBoolean(this._connection.status == .connected);
}
...
fn consumeOnConnectCallback(this: *const @This(), globalObject: *jsc.JSGlobalObject) ?jsc.JSValue {
    if (this._vm.isShuttingDown()) return null;
    if (this._js_value.tryGet()) |value| {
...
pub fn getQueriesArray(this: *@This()) JSValue {
    if (this._vm.isShuttingDown()) return .js_undefined;
    if (this._js_value.tryGet()) |value| {
...
fn failFmt(this: *@This(), error_code: AnyMySQLError.Error, comptime fmt: [:0]const u8, args: anytype) void {
    ...
    const err = AnyMySQLError.mysqlErrorToJS(this._globalObject, message, error_code);
...
pub fn onConnectionEstabilished(this: *@This()) void {
    if (this._vm.isShuttingDown()) return;
    const on_connect = this.consumeOnConnectCallback(this._globalObject) orelse return;
    on_connect.ensureStillAlive();
    var js_value = this._js_value.tryGet() orelse .js_undefined;
    js_value.ensureStillAlive();
    this._globalObject.queueMicrotask(on_connect, &[_]JSValue{ JSValue.jsNull(), js_value });
}
...
pub fn onResultRow(this: *@This(), request: *JSMySQLQuery, statement: *MySQLStatement, Context: type, reader: NewReader(Context)) (error{ ShortRead, JSError })!void {
    ...
    switch (result_mode) {
        .objects => {
            cached_structure = if (this._js_value.tryGet()) |value| statement.structure(value, this._globalObject) else null;
    ...
    row.decode(allocator, reader) catch |err| {
        ...
        this._connection.queue.markCurrentRequestAsFinished(request);
    ...
    if (this._globalObject.tryTakeException()) |err| {
        this._connection.queue.markCurrentRequestAsFinished(request);
...
pub fn onError(this: *@This(), request: ?*JSMySQLQuery, err: AnyMySQLError.Error) void {
    if (request) |_request| {
        ...
        if (this._globalObject.tryTakeException()) |err_| {
    ...
    if (request) |_request| {
        if (this._vm.isShuttingDown()) {
            ...
            if (this._globalObject.tryTakeException()) |err_| {
                ...
                _request.rejectWithJSValue(this.getQueriesArray(), err.toJS(this._globalObject));
            }
        }
    } else {
        if (this._vm.isShuttingDown()) {
        ...
        if (this._globalObject.tryTakeException()) |err_| {
            ...
            this.failWithJSValue(err.toJS(this._globalObject));
        }
    }
}
