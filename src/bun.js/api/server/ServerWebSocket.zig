...
_handler: *WebSocketServer.Handler,
_this_value: jsc.JSRef = .empty(),
_flags: Flags = .{},
_signal: ?*bun.webcore.AbortSignal = null,
...
pub fn onOpen(this: *ServerWebSocket, ws: uws.AnyWebSocket) void {
    ...
    this._flags.opened = false;

    if (onOpenHandler.isEmptyOrUndefinedOrNull()) {
        return;
    }

    const this_value = this._this_value.tryGet() orelse .js_undefined;
...
pub fn onMessage(
    ...
    const onMessageHandler = this._handler.onMessage;
    if (onMessageHandler.isEmptyOrUndefinedOrNull()) return;
    var globalObject = this._handler.globalObject;
    ...
    const arguments = [_]JSValue{
        this._this_value.tryGet() orelse .js_undefined,
        switch (opcode) {
    ...
    if (result.toError()) |err_value| {
        this._handler.runErrorCallback(vm, globalObject, err_value);
        return;
    }
...
pub fn onDrain(this: *ServerWebSocket, _: uws.AnyWebSocket) void {
    ...
    if (handler.onDrain != .zero) {
        ...
        var corker = Corker{
            .args = &[_]jsc.JSValue{this._this_value.tryGet() orelse .js_undefined},
            .globalObject = globalObject,
...
pub fn onPing(this: *ServerWebSocket, _: uws.AnyWebSocket, data: []const u8) void {
    ...
    _ = cb.call(
        globalThis,
        .js_undefined,
        &[_]jsc.JSValue{ this._this_value.tryGet() orelse .js_undefined, this.binaryToJS(globalThis, data) catch .zero }, // TODO: properly propagate exception upwards
    ) catch |e| {
...
pub fn onClose(this: *ServerWebSocket, _: uws.AnyWebSocket, code: i32, message: []const u8) void {
    ...
    defer {
        ...
        if (this._this_value.isNotEmpty()) {
            this._this_value.downgrade();
        }
    }
    ...
    if (!handler.onClose.isEmptyOrUndefinedOrNull()) {
        ...
        const message_js = bun.String.createUTF8ForJS(globalObject, message) catch |e| {
            ...
            log("onClose error (message) {}", .{this._this_value.isNotEmpty()});
            handler.runErrorCallback(vm, globalObject, err);
            return;
        };

        _ = handler.onClose.call(globalObject, .js_undefined, &[_]jsc.JSValue{ this._this_value.tryGet() orelse .js_undefined, JSValue.jsNumber(code), message_js }) catch |e| {
            const err = globalObject.takeException(e);
            log("onClose error {}", .{this._this_value.isNotEmpty()});
            handler.runErrorCallback(vm, globalObject, err);
            return;
        };
    } else if (signal) |sig| {
...
pub fn publish(
    ...
    const app = this._handler.app orelse {
        log("publish() closed", .{});
        return JSValue.jsNumber(0);
    };
    const flags = this._handler.flags;
    const ssl = flags.ssl;
...
pub fn getData(
    ...
    log("getData()", .{});
    if (this._this_value.tryGet()) |this_value| {
